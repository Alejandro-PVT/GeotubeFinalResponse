...
export function LocationMap({ onVideosFound }: LocationMapProps) {
  ...
  useEffect(() => {
    if (coords && circleRef.current && mapInstance.current) {
      drawCircle(coords.lat, coords.lng, radius[0])
    }
  }, [radius])
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div ref={mapRef} className="h-[400px] w-full rounded-lg overflow-hidden border relative z-10" />
      </CardContent>
    </Card>
  )
}
